...
func HandleGraph(s store.Store, log *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if view == "episodes" {
			...
			autoGenerated := map[string]bool{"temporal": true, "similar": true}
			addEdge := func(e GraphEdge) {
				k := makeKey(e.Source, e.Target)
				existing, ok := bestEdge[k]
				if !ok {
					bestEdge[k] = e
					return
				}
				existingIsAuto := autoGenerated[existing.RelationType]
				newIsAuto := autoGenerated[e.RelationType]
				// Explicit associations always beat auto-generated edges.
				...
				// Same category: keep the strongest.
